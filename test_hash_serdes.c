...
Describe(HashSerdes);
BeforeEach(HashSerdes) {}
AfterEach(HashSerdes) {}
...
Ensure(HashSerdes, test_hash_serdes) {
  ...
  assert_that(s, is_equal_to_string("{\"k1\": \"element 1\", \"k2\": \"element 2\", \"num\": -80, \"str\": \"test str\"}"));
  free(s);
}

int main(int argc, char **argv) {
  TestSuite *suite = create_test_suite();
  add_test_with_context(suite, HashSerdes, test_hash_serdes);
